...
void SettingsDisplay(SetMenuTy Menu)
{
    uint8_t i,color,p,lineh,height;
    char msg[10];
    p=Menu.top;
...
//    sprintf(msg,"%d,%d",Menu.page,Menu.pos);
//    lcd_write_string(msg,100,30,SmallFont,WHITE_OVER_BLACK);
/*    if (Menu.pos>lineh*Menu.PageSize*Menu.page)
    {
        Menu.page++;
        Menu.pos=lineh*Menu.PageSize*Menu.page;
        Menu.refresh=True;
    } 
    else if (Menu.pos<lineh*Menu.PageSize*Menu.page)
    {
        Menu.page--;
        Menu.pos=lineh*Menu.PageSize*Menu.page;
        Menu.refresh=True;
    ...
    if (Menu.refresh)
    {
        ...
        for (i=0;i<SettingsMenu.TotMenuItems;i++)
        {
...
SetMenuTy MenuSelection(SetMenuTy Menu)
{
    TBool Done=False;
        ...
        if (Keypressed)
        {
            switch (Key)
            {
                case KeyUp:if (Menu.menu>1) {
                               Menu.prev=Menu.menu;
                               Menu.menu--;
                               if (((Menu.menu-1)*Menu.lineh)<Menu.pos) Menu.pos -=Menu.lineh ;
                               Menu.refresh=False;
                ...
                case KeyDw:if (Menu.menu<(SettingsMenu.TotMenuItems)) {
                               Menu.prev=Menu.menu;
                               Menu.menu++; 
                               if (((Menu.menu-1)*Menu.lineh)>(Menu.height+Menu.pos)) Menu.pos +=Menu.lineh ;
                               Menu.refresh=False;
                               SettingsDisplay(Menu);
                          }
                          else Beep();
...
TBool MenuYNselect(SetMenuTy Menu)
{
    TBool Done=False;
    TBool SetValue;
    Menu.refresh=True;
    ...
    while (!Done)
    {
        if (Keypressed)
        {
            switch (Key)
            {
                case KeyUp:SettingsDisplay(Menu);
                           SetValue=True;
                    break;
                case KeyDw:SettingsDisplay(Menu);
                           SetValue=False;
                    break;
                default: //select and exit
                    Done=True;
            }
            while (Keypressed); // wait here till key is released
        }
...
{
    uint16_t t=0;
    uint8_t Y1=PopY1+((PopY2-(PopY1+MediumFont_height))>>1);
    uint8_t X1=PopX1+((PopX2-(PopX1+lcd_string_lenght(msg,MediumFont)))>>1);
    lcd_fill_block(PopX1,PopY1,PopX2,PopY2);
    lcd_write_string(msg,X1,Y1,MediumFont,WHITE_OVER_BLACK);
    if (wait==0)
    {
        while(!Keypressed);
        return Key;
...
void Diagnostics(void)
{
    ...
    do {
      ...
      if (Menu.menu>0)  Diagnose(Menu.menu);
    } while (Menu.menu>0) ;
}
// </editor-fold>
// </editor-fold>
...
uint8_t  TestBattery(void)
{
    uint16_t battery = ADC_Read(BATTERY);
    uint16_t battery_mV = battery*BAT_divider;
    battery_level = (battery_mV/10)-320; // ((battery_mV-3200)*100)/(3900-3200)
    return  battery_level; // can be over 100
}
// </editor-fold>
                ...
                if (Keypressed) 
                {
                    switch (Key)
                    {
                        ...
                        case KeyDw: if (TotPar>0) 
                                    ...
                                    {
                                        if (ParTime[1]>=(((BuzzerStartDuration/10)+10)+i))
                                        {
                                            ParTime[1] -=i;
                                            changed=True;
                                        }
...
void SetAutoStart()
{
    ...
    Menu.pos=0;
    Menu.menu=1;
    SettingsMenu.TotMenuItems=2;
    strcpy(SettingsMenu.MenuTitle,"Settings: ");
    strcpy(SettingsMenu.MenuItem[0]," Auto Start ON ");
    strcpy(SettingsMenu.MenuItem[1]," Auto Start OFF ");
    
    Menu.top = SettingsTitle();
    Menu.refresh=True;
    SettingsDisplay(Menu); 
    orgset= AutoStart;
    AutoStart= MenuYNselect(Menu);
...
void CountDownMode(uint8_t cdt)
{
    ...
    prev_cdtime=0;
    strcpy(SettingsMenu.MenuTitle,"Countdown ");
    top = SettingsTitle();
    while (!Exit)
    {
        cdtime=cdt*10;
...
void Settings(void)
{
    ...
    do {
        ...
        SettingsDisplay(Menu);
        Menu =MenuSelection(Menu);
        if (Menu.menu>0)  DoSet(Menu.menu);
    } while (Menu.menu>0);
    if (SaveToEEPROM)
    {
        saveSettings();
...
// <editor-fold defaultstate="collapsed" desc="Main">
    ...
    __delay_ms(100);
    generate_sinus(2,1000,1000); 
    __delay_ms(100);
    generate_sinus(2,1500,1000); 
    __delay_ms(100);
    generate_sinus(2,2000,1000);  
    __delay_ms(100);
    generate_sinus(2,2500,1000); 
    __delay_ms(100);
    generate_sinus(2,3000,1000); 
    __delay_ms(100);
    while(True) 
    {
        if (Powered)
